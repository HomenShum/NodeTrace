...
const P=dirname(fileURLToPath(import.meta.url)),R='D:/VSCode Projects/cafecorner_nodebench/nodebench_ai4/NodeTrace',C='C:/Users/hshum/AppData/Local/Temp/nodetrace-next-e2e-lU78sE',O=join(P,'E6e-nodetrace-disclosure-entry-before-03');mkdirSync(O);
 ...
 for(const surface of ['source','installed']){
  ...
  for(const [width,height]of surface==='source'?[[320,800],[390,844]]:[[320,800]]){
   ...
   if(surface==='source'&&width===390){
    ...
    const escapeUrl=url;writeFileSync(join(O,'existing-coach-inspection-route.json'),JSON.stringify({before:url,open:page.url(),expectedAfter:escapeUrl},null,2));await page.keyboard.press('Escape');await dialog.waitFor({state:'hidden'});check('Existing valid coach inspection closes and restores pre-inspection URL',!await dialog.isVisible()&&page.url()===escapeUrl);
   }
   await context.close();
  }
