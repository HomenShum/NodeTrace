...
const P=dirname(fileURLToPath(import.meta.url)),R='D:/VSCode Projects/cafecorner_nodebench/nodebench_ai4/NodeTrace',C='C:/Users/hshum/AppData/Local/Temp/nodetrace-next-e2e-lU78sE',O=join(P,'E6e-nodetrace-disclosure-entry-before');mkdirSync(O);
 ...
 for(const surface of ['source','installed']){
  ...
  for(const [width,height]of surface==='source'?[[320,800],[390,844]]:[[320,800]]){
   ...
   if(surface==='source'&&width===390){
    ...
    const escapeUrl=page.url();await page.keyboard.press('Escape');check('Existing valid coach inspection closes without URL mutation',!await dialog.isVisible()&&page.url()===escapeUrl);
   }
   await context.close();
  }
