...
const out=join(packet,'E6c-nodetrace-matched-canonical-visual-retry-02');mkdirSync(out);
...
try{
 ...
 for(const fixture of [{label:'source',original:originalPrimary,origin:'http://127.0.0.1:4917',route:'/?unrelated=preserved#review'},{label:'installed',original:originalInstalled,origin:'http://127.0.0.1:54274',route:'/nodetrace?host=preserved&returnTo=dashboard#host-section'}]){
  ...
  for(const [width,height]of [[1440,960]]){
   ...
   if(!(await metadata(page)).frame.length){const started=Date.now();await page.waitForFunction(()=>window.__nodeTraceEntityFrame().length>0,{},{timeout:2000});cell.currentFrameWaitMs=Date.now()-started;await page.evaluate(()=>new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r))));}
