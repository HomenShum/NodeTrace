...
const out=join(packet,'E6c-nodetrace-matched-canonical-visual-retry-01');mkdirSync(out);
...
try{
 ...
 for(const fixture of [{label:'source',original:originalPrimary,origin:'http://127.0.0.1:4917',route:'/?unrelated=preserved#review'},{label:'installed',original:originalInstalled,origin:'http://127.0.0.1:54274',route:'/nodetrace?host=preserved&returnTo=dashboard#host-section'}]){
  ...
  if(fixture.label!=='installed')continue;
  for(const [width,height]of [[1440,960]]){
   ...
   writeFileSync(join(out,tag+'-pre-assert.json'),JSON.stringify(await metadata(page),null,2));writeFileSync(join(out,tag+'-pre-assert.png'),await page.screenshot());
   ...
   const split=target.indexOf(': ');check(tag+' both selection panels agree',(await detail.locator('dd').first().textContent()).trim()===target.slice(split+2)+' Â· '+target.slice(0,split));check(tag+' measured count stays unknown',await detail.getByTestId('count-readout').textContent()==='unknown â€” not measured');
