...
// Both pointer callbacks and the visible detail use the current input snapshot.
...
export function NodeGraph({ nodes, edges, visits, dark = false, height = 480, kindColors, selectedNodeId, onNode, onContext, }) {
    ...
    const selectionInputs = useRef({ nodes, edges, on });
    selectionInputs.current = { nodes, edges, on };
    const selected = nodeMessage(nodes, edges, on, selectedNodeId === undefined ? selectedId : selectedNodeId);
    ...
    useEffect(() => {
        ...
        const emitNode = (id) => {
            ...
            const msg = nodeMessage(current.nodes, current.edges, current.on, id);
            if (!msg) return;
            setSelectedId(id);
