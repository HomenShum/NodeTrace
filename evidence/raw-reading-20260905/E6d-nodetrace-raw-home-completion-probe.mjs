...
try {
  ...
  for (const [fixture, width, height, motion = 'no-preference', doubled = false] of cases) {
    ...
    if (!consumer && fixture === 'long' && width === 390 && motion === 'no-preference') {
      ...
      await capture(page, `${name}-sustained`); writeFileSync(join(out,'native-completed-reading.json'),JSON.stringify(await page.evaluate(()=>({state:window.__rawState(),events:window.__rawDiag,frames:window.__rawFrames})),null,2));
    }
    await context.close();
  }
