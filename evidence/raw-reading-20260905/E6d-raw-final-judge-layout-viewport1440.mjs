...
const O=join(P,'E6d-nodetrace-raw-final-judge-layout-viewport1440');mkdirSync(O);
...
async function save(page,name,m){const data={...m,rawText:undefined,rawSha256:sha(m.rawText)};writeFileSync(join(O,name+'.json'),JSON.stringify(data,null,2));writeFileSync(join(O,name+'.html'),await page.content());await page.screenshot({path:join(O,name+'.png'),fullPage:false});return data}
try{
 ...
 const initial=await metrics(page);await save(page,`${width}-current-initial`,initial);const postCapture=await metrics(page); check(width+': viewport capture preserves every measured field',JSON.stringify(initial)===JSON.stringify(postCapture));
...
}catch(e){failure=e.stack;console.error(failure);if(active&&!active.isClosed()){writeFileSync(join(O,'failure.html'),await active.content());await active.screenshot({path:join(O,'failure.png'),fullPage:false})}}
