...
const R='D:/VSCode Projects/cafecorner_nodebench/nodebench_ai4/NodeTrace',P=dirname(fileURLToPath(import.meta.url)),O=join(P,'E6d-nodetrace-raw-retry-judge-03');
...
const before=hashes(),head=git('rev-parse','HEAD').trim(),index=sha(git('ls-files','--stage','-z')); writeFileSync(join(O,'custody-before.json'),JSON.stringify({before,head,index},null,2));
...
const after=hashes(); writeFileSync(join(O,'custody-after.json'),JSON.stringify({after,head:git('rev-parse','HEAD').trim(),index:sha(git('ls-files','--stage','-z'))},null,2)); check('source build public HEAD and index inputs unchanged',JSON.stringify(before)===JSON.stringify(after)&&git('rev-parse','HEAD').trim()===head&&sha(git('ls-files','--stage','-z'))===index);
